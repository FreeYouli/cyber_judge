...
class JudgmentPanel(QWidget):
    ...
    def show_result(self, j, a_text, b_text):
        ...
        lo.addWidget(btn, alignment=Qt.AlignLeft)
    ...
    def _build_html(self, j, a_text, b_text):
        ...
        h += f'<p style="font-size:13pt;color:#2A3A4A;line-height:1.6;margin:8px 0 16px 0;">'
        ...
        if fallacies:
            ...
            for f in fallacies:
                ...
                h += f'<p style="font-size:12pt;color:#2A3A4A;margin:4px 0;">'
        ...
        if rs_desc:
            h += f'<p style="font-size:12pt;color:#4A5A6A;margin:4px 0 12px 0;">{rs_desc}</p>'

        sugg = html_mod.escape(j.get("suggestions", ""))
        if sugg:
            h += f'<p style="font-size:13pt;color:#2A3A4A;padding:10px;'
            h += f'background:#FFF8E8;border-radius:6px;margin:8px 0;">💡 {sugg}</p>'

        # 免责声明
        h += '<hr style="border:none;border-top:1px solid #E0E0E0;margin:16px 0;">'
        h += '<p style="font-size:10pt;color:#A0AAB0;">* 判决由AI生成，仅供参考</p>'

        h += '</div>'
        return h

    def _section(self, title):
        return (f'<h3 style="color:#1A2A4A;font-size:14pt;font-weight:bold;'
                f'margin:16px 0 4px 0;">{title}</h3>'
                f'<hr style="border:none;border-top:2px solid #D0D8E4;margin:0 0 8px 0;">')
    ...
    def _sentiment_bar(self, label, sentiment):
        ...
        h = f'<p style="font-size:11pt;color:#4A5A6A;margin:6px 0 2px 0;">{label}：{label_text}</p>'
        ...
        h += f'<td bgcolor="#E8EDF4" height="20" align="right">'
        h += f'<font color="#666" size="2">{s:.2f}</font></td>'
        h += '</tr></table>'
        return h
